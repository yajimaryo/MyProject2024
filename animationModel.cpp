...
D3DXVECTOR3 AnimationModel::GetBonePosition(char * BoneName)
{
	bool boneFound = false;

	// ボーン名と一致するボーンを探す
	for (auto& pair : m_Bone) {
		if (strcmp(pair.first.c_str(), BoneName) == 0) {
			// ボーン名が一致した場合、その座標を取得する
			boneFound = true;
			float x = pair.second.Matrix.a1;
			float y = -pair.second.Matrix.b1;
			float z = pair.second.Matrix.c1;
			return D3DXVECTOR3(x, y, z);
			
		}
	}

	// ボーン名が見つからない場合、適切なエラーハンドリングを行う
	if (!boneFound) {
		// 例えばエラーメッセージを出力するか、デフォルトの座標を返すなどの処理を行う
		// ここではデフォルトの座標を返していますが、適切なエラーハンドリングが必要です
	...
	const char *AnimationName2, int Frame2, float BlendRate)
{
	...
	for (auto pair : m_Bone)
	{
		...
		for (unsigned int c = 0; c < animation1->mNumChannels; c++)
		{
			...
			{
				nodeName = animation1->mChannels[c]->mNodeName.C_Str();
...
void AnimationModel::UpdateBoneMatrix(aiNode* node, aiMatrix4x4 matrix)
{
	...
	if (&m_Bone[node->mName.C_Str()] == &m_Bone["mixamorig:LeftFoot"])
	{
		Scene* scene = Manager::GetInstance()->GetScene();
		MeshField* mf = scene->GetGameObject<MeshField>();

		D3DXVECTOR3 footPos = D3DXVECTOR3(bone->Matrix.a1,
			bone->Matrix.b1,
			bone->Matrix.c1);

		bone->Matrix.b1 += mf->GetHeight(footPos) + 0.1f;
	}
